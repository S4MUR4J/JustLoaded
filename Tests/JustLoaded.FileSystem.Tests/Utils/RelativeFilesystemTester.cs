...
public abstract class RelativeFilesystemTester<TSource>
    : FilesystemTester<RelativeFilesystem, TSource>
    where TSource : IRelativeFilesystemTestSource, new()
{
...
{
    public IEnumerable SourceSingleFileRelative =>
        GetSingleFileRelativeSource.Select(file => new TestCaseData(file.file, file.query));
...
public abstract class RelativeFilesystemTestSourceBase : IRelativeFilesystemTestSource
{
    ...
    public IEnumerable<ModAssetPath> GetFileFlatFileName =>
        Enumerable.Repeat("".AsPath().FromAnyMod(), 1);
    public IEnumerable<ModAssetPath[]> GetFilesFlatFileNames =>
        Enumerable.Repeat(new[] { "".AsPath().FromAnyMod() }, 1);

    public abstract IEnumerable<(
        ModAssetPath file,
        ModAssetPath query
    )> GetSingleFileRelativeSource { get; }
    public abstract IEnumerable<(
        ModAssetPath query,
        ModAssetPath[] files,
        ModAssetPath[] results
    )> GetListDirsSource { get; }
    public abstract IEnumerable<(
        ModAssetPath query,
        ModAssetPath[] files,
        ModAssetPath[] results
    )> GetListFilesShallowSource { get; }
    public abstract IEnumerable<(
        ModAssetPath query,
        ModAssetPath[] files,
        ModAssetPath[] results
    )> GetListFilesRecursiveSource { get; }
    public abstract IEnumerable<(
        string pattern,
        ModAssetPath[] files,
        ModAssetPath[] results
    )> GetListFilesPatternSource { get; }
}
